...
test.serial('creates a token', async t => {
  const stdout = await runAragonCLI(['dao', 'token', 'new', 'Member token', 'MBR', 0], verbose)
  TOKEN = matchAddressAtLineContaining(stdout, 'Successfully deployed the token')
...
test.serial('installs a token manager app', async t => {
  const stdout = await runAragonCLI(['dao', 'install', DAO, 'token-manager', '--app-init', 'none'], verbose)
  TOKEN_MANAGER = matchAddressAtLineContaining(stdout, 'Installed token-manager')
...
test.serial('installs the voting app', async t => {
  const stdout = await runAragonCLI(['dao', 'install', DAO, 'voting', '--app-init-args', TOKEN, '600000000000000000', '250000000000000000', '604800'], verbose)
...
test.serial('installs the finance app', async t => {
  const stdout = await runAragonCLI(['dao', 'install', DAO, 'finance', '--app-init-args', VAULT, '2592000'], verbose)
