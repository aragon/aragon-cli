...
> Snapshot 1
    ...
    → npm run prepublish: ␊
    > foobar@1.0.0 prepublish /home/daniel/r/ar/aragon-cli/packages/e2e-tests/.tmp/foobar␊
    > aragon contracts compile␊
    ␊
    ␊
    → npm run prepublish:  i Passing the command to Truffle␊
    ␊
    Running prepublish script [completed]␊
