...
interface IACL {
    ...
    function hasPermission(address who, address where, bytes32 what, bytes how)
        public
        view
        returns (bool);
}
...
contract ACLSyntaxSugar {
    ...
    function arr(address _a, uint256 _b, uint256 _c)
        internal
        pure
        returns (uint256[] r)
    {
    ...
    function arr(address _a, uint256 _b, uint256 _c, uint256 _d)
        internal
        pure
        returns (uint256[] r)
    {
    ...
    function arr(address _a, address _b, uint256 _c, uint256 _d, uint256 _e)
        internal
        pure
        returns (uint256[] r)
    {
    ...
    function arr(address _a, address _b, address _c)
        internal
        pure
        returns (uint256[] r)
    {
    ...
    function arr(address _a, address _b, uint256 _c)
        internal
        pure
        returns (uint256[] r)
    {
    ...
    function arr(uint256 _a, uint256 _b, uint256 _c)
        internal
        pure
        returns (uint256[] r)
    {
    ...
    function arr(uint256 _a, uint256 _b, uint256 _c, uint256 _d)
        internal
        pure
        returns (uint256[] r)
    {
    ...
    function arr(uint256 _a, uint256 _b, uint256 _c, uint256 _d, uint256 _e)
        internal
        pure
        returns (uint256[] r)
    {
...
contract ACLHelpers {
    ...
    function decodeParamsList(uint256 _x)
        internal
        pure
        returns (uint32 a, uint32 b, uint32 c)
    {
        a = uint32(_x);
        b = uint32(_x >> (8 * 4));
        c = uint32(_x >> (8 * 8));
...
contract Initializable is TimeHelpers {
    ...
    */
    function hasInitialized() public view returns (bool) {
        ...
        return
            initializationBlock != 0 && getBlockNumber() >= initializationBlock;
    }
...
library SafeERC20 {
    ...
    */
    function safeTransfer(ERC20 _token, address _to, uint256 _amount)
        internal
        returns (bool)
    {
    ...
    *      Note that this makes an external call to the token.
    */
    function safeApprove(ERC20 _token, address _spender, uint256 _amount)
        internal
        returns (bool)
    {
    ...
    */
    function staticBalanceOf(ERC20 _token, address _owner)
        internal
        view
        returns (uint256)
    {
        ...
        (bool success, uint256 tokenBalance) = staticInvoke(
            _token,
            balanceOfCallData
        );
    ...
    * Reverts if the call fails for some reason (should never fail).
    */
    function staticAllowance(ERC20 _token, address _owner, address _spender)
        internal
        view
        returns (uint256)
    {
        ...
        (bool success, uint256 allowance) = staticInvoke(
            _token,
            allowanceCallData
        );
...
interface IEVMScriptExecutor {
    function execScript(bytes script, bytes input, address[] blacklist)
        external
        returns (bytes);
    ...
    ACLSyntaxSugar
{
    ...
    */
    function canPerform(address _sender, bytes32 _role, uint256[] _params)
        public
        view
        returns (bool)
    {
    ...
    */
        ...
        authP(TRANSFER_ROLE, arr(_token, _to, _value))
    {
        ...
        if (_token == ETH) {
            ...
            require(
                ERC20(_token).safeTransfer(_to, _value),
...
contract Finance is EtherTokenConstant, IsContract, AragonApp {
    ...
    bytes32 public constant CREATE_PAYMENTS_ROLE = keccak256(
        'CREATE_PAYMENTS_ROLE'
    );
    bytes32 public constant CHANGE_PERIOD_ROLE = keccak256(
        'CHANGE_PERIOD_ROLE'
    );
    bytes32 public constant CHANGE_BUDGETS_ROLE = keccak256(
        'CHANGE_BUDGETS_ROLE'
    );
    bytes32 public constant EXECUTE_PAYMENTS_ROLE = keccak256(
        'EXECUTE_PAYMENTS_ROLE'
    );
    bytes32 public constant MANAGE_PAYMENTS_ROLE = keccak256(
    ...
    modifier transitionsPeriod {
        bool completeTransition = _tryTransitionAccountingPeriod(
            getMaxPeriodTransitions()
        );
    ...
    modifier scheduledPaymentExists(uint256 _paymentId) {
        require(
            _paymentId > 0 && _paymentId < paymentsNextIndex,
            ERROR_NO_SCHEDULED_PAYMENT
        );
        _;
    }
    ...
    modifier transactionExists(uint256 _transactionId) {
        require(
            _transactionId > 0 && _transactionId < transactionsNextIndex,
    ...
    */
    function initialize(Vault _vault, uint64 _periodDuration)
        external
        onlyInit
    {
        initialized();
    ...
    */
        ...
        transitionsPeriod
    {
        ...
        _deposit(_token, _amount, _reference, msg.sender, true);
    }
    ...
    */
        ...
        returns (uint256 paymentId)
    {
        ...
        require(
            !settings.hasBudget[_token] || settings.budgets[_token] >= _amount,
            ERROR_BUDGET
        );
        ...
        if (_maxExecutions == 1) {
            require(
                _initialPaymentTime > getTimestamp64(),
                ERROR_NEW_PAYMENT_IMMEDIATE
            );
        }
    ...
    */
    function setPeriodDuration(uint64 _periodDuration)
        ...
        authP(
            CHANGE_PERIOD_ROLE,
            arr(uint256(_periodDuration), uint256(settings.periodDuration))
        )
        transitionsPeriod
    {
        require(_periodDuration >= MINIMUM_PERIOD, ERROR_SET_PERIOD_TOO_SHORT);
    ...
    function setBudget(address _token, uint256 _amount)
        ...
        authP(
            ...
            arr(
                _token,
                _amount,
                settings.budgets[_token],
                uint256(settings.hasBudget[_token] ? 1 : 0)
            )
        )
        transitionsPeriod
    {
    ...
    */
    function executePayment(uint256 _paymentId)
        ...
        authP(
            EXECUTE_PAYMENTS_ROLE,
            arr(_paymentId, scheduledPayments[_paymentId].amount)
        )
    ...
    */
    ...
    {
        require(
            scheduledPayments[_paymentId].receiver == msg.sender,
            ERROR_PAYMENT_RECEIVER
        );
        _executePaymentAtLeastOnce(_paymentId);
    }
    ...
    */
    ...
    function recoverToVault(address _token)
        ...
        transitionsPeriod
    {
        uint256 amount = _token == ETH
            ? address(this).balance
            : ERC20(_token).staticBalanceOf(address(this));
        require(amount > 0, ERROR_RECOVER_AMOUNT_ZERO);

        _deposit(_token, amount, 'Recover to Vault', address(this), false);
    }
    ...
    */
    function tryTransitionAccountingPeriod(uint64 _maxTransitions)
        external
        isInitialized
    ...
    */
    ...
    {
        TokenStatement storage tokenStatement = periods[_periodId]
            .tokenStatement[_token];
    ...
    */
    function getBudget(address _token)
        public
        view
        isInitialized
        returns (uint256 budget, bool hasBudget)
    {
    ...
    * @dev We have to check for initialization as budgets are only valid after initializing
    */
    function getRemainingBudget(address _token)
        public
        view
        isInitialized
        returns (uint256)
    ...
    * @dev We have to check for initialization as budgets are only valid after initializing
    */
    function canMakePayment(address _token, uint256 _amount)
        public
        view
        isInitialized
        returns (bool)
    ...
    *      scheduled payments can only be created via `newScheduledPayment(),` which requires initialization
    */
    function nextPaymentTime(uint256 _paymentId)
        public
        view
        scheduledPaymentExists(_paymentId)
        returns (uint64)
    ...
    function _deposit(
        ...
        _recordIncomingTransaction(_token, _sender, _amount, _reference);
    ...
    function _executePayment(uint256 _paymentId) internal returns (uint256) {
        ...
        uint64 paid = 0;
        while (
            _nextPaymentTime(_paymentId) <= getTimestamp64() &&
            paid < MAX_SCHEDULED_PAYMENTS_PER_TX
        ) {
    ...
    */
    ...
    function _recordTransaction(
        ...
        TokenStatement storage tokenStatement = periods[periodId]
            .tokenStatement[_token];
        if (_incoming) {
    ...
    function _tryTransitionAccountingPeriod(uint64 _maxTransitions)
        internal
        returns (bool success)
    {
    ...
    function _canMakePayment(address _token, uint256 _amount)
        internal
        view
        returns (bool)
    {
        return
            _getRemainingBudget(_token) >= _amount &&
            vault.balance(_token) >= _amount;
    }
    ...
    function _getRemainingBudget(address _token)
        internal
        view
        returns (uint256)
    {
        ...
        uint256 spent = periods[_currentPeriodId()].tokenStatement[_token]
            .expenses;
    ...
    function _nextPaymentTime(uint256 _paymentId)
        internal
        view
        returns (uint64)
    {
    ...
    function getMaxPeriodTransitions() internal view returns (uint64) {
        return MAX_UINT64;
    }
}
