...
exports.handler = async function ({ reporter, module, network, apm: apmOptions }) {
  ...
  const tasks = new TaskList([
    ...
    },
    {
      ...
      task: async (ctx, task) => {
        ...
        ctx.versions = await Promise.all(e.map(async ev => apm.getLatestVersion(ev.returnValues.id)))
      }
    }
  ])
  ...
  return tasks.run()
    .then((ctx) => {
      const repoId = module.appName.split('.').slice(1).join('.')
